   ...
   */
    ...
    */
    ...
    {    
        ...
        var totlines = sentence.length;
        for(var i = 0; i < totlines; i++){
            var line = sentence[i].split(/\b\s+(?!$)/);
            if(line[0] == "v"){
                this.vBuffer.push(Number(line[1]));
                this.vBuffer.push(Number(line[2]));
                this.vBuffer.push(Number(line[3]));
            }
            else if(line[0] == "f"){
                this.fBuffer.push(Number(line[1]) - 1);
                this.fBuffer.push(Number(line[2]) - 1);
                this.fBuffer.push(Number(line[3]) - 1);
            }
        }
    ...
    */    
    ...
    canonicalTransform(){
        //Your code here
        let L = Math.max(this.maxXYZ[0] - this.minXYZ[0], 
                Math.max(this.maxXYZ[1] - this.minXYZ[1], 
                         this.maxXYZ[2] - this.minXYZ[2]));

        let midpoint = [-(this.maxXYZ[0] + this.minXYZ[0]) / 2.0, 
                        -(this.maxXYZ[1] + this.minXYZ[1]) / 2.0, 
                        -(this.maxXYZ[2] + this.minXYZ[2]) / 2.0];

        var S = glMatrix.mat4.create(), T = glMatrix.mat4.create();
        glMatrix.mat4.fromTranslation(T, midpoint)
        glMatrix.mat4.fromScaling(S, [1.0/L, 1.0/L, 1.0/L])
